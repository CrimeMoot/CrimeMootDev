...
namespace Content.Client.HealthAnalyzer.UI
{
    ...
    public sealed partial class HealthAnalyzerWindow : FancyWindow
    {
        ...
        public void Populate(HealthAnalyzerScannedUserMessage msg)
        {
            ...
            var showAlerts = msg.Unrevivable == true || msg.Bleeding == true || msg.CerebralTrauma.HasValue;
            ...
            // Отображение церебральной травмы в анализаторе здоровья
            if (msg.CerebralTrauma.HasValue)
            {
                var traumaText = msg.CerebralTrauma.Value switch
                {
                    CerebralTraumaSeverity.Light => Loc.GetString("health-analyzer-window-entity-cerebral-trauma-light"),
                    CerebralTraumaSeverity.Serious => Loc.GetString("health-analyzer-window-entity-cerebral-trauma-serious"),
                    _ => null
                };

                if (traumaText != null && msg.GrantedQuirkIds != null && msg.GrantedQuirkIds.Count > 0)
                {
                    var quirkNames = new List<string>();
                    foreach (var quirkId in msg.GrantedQuirkIds)
                    {
                        if (_prototypes.TryIndex<TraitPrototype>(quirkId, out var traitProto))
                        {
                            quirkNames.Add(Loc.GetString(traitProto.Name));
                        }
                    }
                    
                    if (quirkNames.Count > 0)
                    {
                        var quirkList = ContentLocalizationManager.FormatList(quirkNames);
                        traumaText = $"{traumaText}: {quirkList}";
                    }
                }

                if (traumaText != null)
                {
                    var coloredText = $"[color=red]{traumaText}[/color]";
